...
#include "PrefiringStudies/PrefiringAnalyzer/interface/Tools.h"
...
//#include "PrefiringStudies/PrefiringAnalyzer/python/RochesterCorrections/RoccoR.h"
#include "PrefiringStudies/PrefiringAnalyzer/interface/RoccoR.h"
...
class PrefiringAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources,edm::one::WatchRuns>  {
   public:
      explicit PrefiringAnalyzer(const edm::ParameterSet&);
      ~PrefiringAnalyzer();
...
//
PrefiringAnalyzer::PrefiringAnalyzer(const edm::ParameterSet& iConfig)
 :
...
PrefiringAnalyzer::~PrefiringAnalyzer()
{
...
void PrefiringAnalyzer::beginRun(edm::Run const & iRun, edm::EventSetup const& iSetup)
{
...
void PrefiringAnalyzer::endRun(edm::Run const & iRun, edm::EventSetup const& iSetup)
{
...
void
PrefiringAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
void
PrefiringAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
bool PrefiringAnalyzer::GetMETFilterDecision(const edm::Event& iEvent,edm::Handle<TriggerResults> METFilterResults, TString studiedfilter){
...
bool PrefiringAnalyzer::GetIdxFilterDecision(int it){
...
TString PrefiringAnalyzer::GetIdxFilterName(int it){
  if(it==idx_Flag_goodVertices)return "Flag_goodVertices";
  else if(it== idx_Flag_globalTightHalo2016Filter)return "Flag_globalTightHalo2016Filter";
  else if(it== idx_Flag_globalSuperTightHalo2016Filter)return "Flag_globalSuperTightHalo2016Filter";
  else if(it== idx_Flag_HBHENoiseFilter)return "Flag_HBHENoiseFilter";
...
void PrefiringAnalyzer::InitandClearStuff(){
  _mll=0;
  _ptll=0;
  _pzll=0;
  _yll=0;
...
bool PrefiringAnalyzer::PassSkim(){
...
void
PrefiringAnalyzer::CalcDileptonInfo(const int& i, const int& j, Float_t & themass, Float_t & theptll, Float_t & thepzll,  Float_t & theyll, Float_t & thedphill, Float_t & thecosthll){
...
void
PrefiringAnalyzer::CalcDileptonInfoGen(const int& i, const int& j, Float_t & themass, Float_t & theptll, Float_t & thepzll,  Float_t & theyll, Float_t & thedphill, Float_t & thecosthll){
...
DEFINE_FWK_MODULE(PrefiringAnalyzer);
